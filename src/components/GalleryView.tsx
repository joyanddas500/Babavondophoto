...
export const GalleryView: React.FC<GalleryViewProps> = ({
  ...
  // Toggle photo in client's favorite/selection collection
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] text-stone-900 flex flex-col font-sans pb-24 selection:bg-rose-500 selection:text-white">
      ...
      <header className="sticky top-0 z-30 bg-[#FAF8F5]/95 backdrop-blur-md border-b border-stone-200/90 py-3 px-4 sm:px-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl border border-rose-300/80 bg-rose-50/80 flex items-center justify-center text-rose-600 shadow-2xs">
                <Camera className="w-4 h-4" />
              </div>
              <div>
                <span className="font-cinzel font-bold text-stone-900 text-sm sm:text-base tracking-[0.18em] uppercase">
                  {gallery.title}
                </span>
                <p className="text-[8px] sm:text-[9px] font-bold text-rose-700 tracking-[0.25em] uppercase">
          ...
          <div className="hidden lg:flex items-center gap-2 text-stone-600">
            <a
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-stone-300 rounded-xl text-xs font-mono-code font-bold uppercase text-stone-700 hover:bg-stone-100 transition-colors"
            >
            ...
            {gallery.password && (
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1.5 bg-white border border-stone-200 hover:bg-stone-100 rounded-xl text-[10px] font-mono-code uppercase text-stone-600 transition-colors"
                title="Lock portal"
              >
                <span>Lock</span>
              </button>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border border-stone-300 hover:border-rose-400 rounded-xl text-xs font-mono-code font-bold uppercase text-stone-800 shadow-2xs hover:shadow-xs transition-all"
            >
              <Heart className={`w-3.5 h-3.5 ${selectedPhotos.length > 0 ? 'fill-rose-500 text-rose-500' : 'text-stone-400'}`} />
              <span>SELECTED ({selectedPhotos.length})</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-12 pb-8 text-center space-y-3">
        <span className="text-[10px] sm:text-xs font-mono-code font-bold uppercase tracking-[0.25em] text-[#0d766e]">
          CURATE COLLECTION
        </span>

        <h1 className="font-cinzel text-3xl sm:text-4xl md:text-5xl font-bold tracking-[0.1em] text-stone-900 leading-tight">
          SELECT YOUR{' '}
